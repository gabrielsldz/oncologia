...
TIPOS_TXT = Path(__file__).with_name("tipos.txt")
CID_NOMES: dict[str, str] = {}
if TIPOS_TXT.exists():
    _rx = re.compile(r"^([CD]\d{2})\s+[\u2013-]\s+(.*)$")
...
CODES_LIST = [f"{c} – {CID_NOMES[c]}" for c in CID_NOMES]
...
if modo == "Simples":
    ano = st.number_input("Ano", 2013, 2025, 2023, 1)
    ...
    c1, c2 = st.columns(2)
    with c1:
        ano_ini = st.number_input("Ano inicial", 2013, 2025, 2019, 1)
    with c2:
        ano_fim = st.number_input("Ano final", 2013, 2025, 2023, 1)
    if ano_fim < ano_ini:
    ...
    for i in range(n_cons):
        ...
        with col1:
            year = st.number_input(
                f"Ano {i+1}", 2013, 2025, max(2013, 2023 - i), 1, key=f"yr{i}"
            )
        with col2:
    ...
    if st.button("🔀 Comparar"):
        ...
        if not df_cmp.empty and all(is_numeric_dtype(df_cmp[c]) for c in df_cmp):
            ...
            for j, col in enumerate(df_cmp.columns):
                ax.bar([i - 0.4 + width / 2 + j * width for i in x], df_cmp[col], width, label=col)
